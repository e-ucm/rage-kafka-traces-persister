...
if (process.env.NODE_ENV === 'test') {
    ...
    Async.auto({
        projectName: function (done) {
            var promptOptions = {
                default: defaultValues.projectName || 'Survey Manager'
            };
        ...
        port: ['companyName', function (done) {

            var promptOptions = {
                default: defaultValues.port || 3003
            };

            Promptly.prompt('Port: (' + promptOptions.default + ')', promptOptions, done);
        }],
        appPrefix: ['port', function (done) {

            var promptOptions = {
                default: defaultValues.appPrefix || 'kafkaPersister'
            };
